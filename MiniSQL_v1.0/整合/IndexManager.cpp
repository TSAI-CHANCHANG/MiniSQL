...
int BPLUSTREE::CreateIndex(string *file_name, int type) {
	...
	ifstream tmp(*file_name, ios::app);
	tmp.close();
...
//	system(cmd.c_str());
    rename(file_name->c_str(), "old.idx");
        ...
        while (p->key.size() < degree / 2) {
            ...
            if (pre_node && pre_node->key.size() + p->key.size() < degree) {
                ...
                if (p->offset.size() == 0) {
                    ...
                    } else {//p is not the first pointer of its father
                        int i = 0;
                ...
                } else//Merge non-leaf nodes
                {
                    ...
                    p->pointer.clear();
                    pre_node = p;
                    if (p->father != pre_node->father) {//p is the first pointer of its father
                        p = p->father;
                        p->key.erase(p->key.begin());
                        p->pointer.erase(p->pointer.begin());
                    } else {//p is not the first pointer of its father
