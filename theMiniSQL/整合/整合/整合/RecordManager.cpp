...
bool RecordManager::insertRecord(string rawValues)
{
	...
	if (resultRecord != "")
	{
		//if (blocks[blockNum].fileoffset==2) cout<<blocks[blockNum].content<<endl;
	...
	if (resultRecord != "")
	{
		for (IndexInfo indexInfo : indexInfos)
		{
			...
			if (indexInfo.value[0] == '\'') indexInfo.value = indexInfo.value.substr(1, indexInfo.value.length() - 2);
			bPlusTree.Insert(indexInfo.type, &indexFile, &indexInfo.value, blocks[blockNum].fileoffset, blockOffset);
		}
	}


	if (bufferMgr.ErrorInfo == NO_SUCH_BLOCK)
	{
		tableInfo.blocknum++;
		catalogMgr.SetTable(tableInfo);
	}
	return true;
}
...
//        buffermanager bufferMgr;
		...
		for (int i = 0; i < tableInfo.blocknum + 1; i++)
		{
			int blockNum = bufferMgr.FindBlockinBuffer(tableName + ".rec", i); // TODO: fetch all block, connect with catalog
			...
			bufferMgr.DirtBlock(blockNum);
		}
		
		for (auto attr = tableInfo.Attr.begin(); attr != tableInfo.Attr.end(); attr++)
		{
			if (attr->indexname != "noindex")
			{
				string str = attr->indexname + ".idx";
				bPlusTree.DropIndex(&str, attr->type);
				bPlusTree.CreateIndex(&str, attr->type);
			}
		}
