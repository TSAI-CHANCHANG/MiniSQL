...
void catalogmanager::SetTable(Table &tab) {
    ...
    int i = 0;
    int blocknum = buf.FindBlockinBuffer(tab.getName() + ".tab",
                                         i);//从tab中读入第一块，在buffermanager中，这一块的filename blocknumber都已经替换好了
    ...
    for (int j = 0; j < tab.Attrnum; j++) {
        ...
        strout << ' ' << tab.Attr[j].attrname << ' ' << tab.Attr[j].type << ' ' << tab.Attr[j].length << ' '
               << tab.Attr[j].primary
...
int Attribute::defAtt(string attDef)//处理表的属性定义
{
    ...
    if (attDef.substr(pos1, 3) == "int") {
        this->type = TYPE_INT;
        this->length = 0;
    } else if (attDef.substr(pos1, 5) == "float") {
        this->type = TYPE_FLOAT;
        this->length = 0;
    } else if (attDef.substr(pos1, 4) == "char") {
        this->type = TYPE_CHAR;
